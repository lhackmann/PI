...
class ProfissionalController {
  ...
  static async createProfissional(req, res) {
    const { idProfissional, nome, cpf, rg, profissao, ccr, assinatura } = req.body;
    if (!idProfissional || !nome || !cpf || !rg || !profissao || !ccr || !assinatura) {
      res.status(400).json({error: 'ID, Nome, cpf, rg, profissão, ccr e assinatura são obrigatórios!!!',});
      return;
    }

    const createdProfissional = await Profissional.create({idProfissional, nome,cpf,rg,profissao, ccr,assinatura,});
  ...
  static async destroyProfissional(req, res) {
    ...
    await Profissional.destroy({ where: { idProfissional: profissional.idProfissional }})
  ...
  static async updateProfissional(req, res) {
    ...
    const updatedProfissional = await Profissional.update({idProfissional, nome, cpf, rg, profissao, ccr, assinatura },{ where: { id: profissional.id }})
