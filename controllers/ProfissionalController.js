  ...
  static async createProfissional(req, res) {
    ...
    if (!id || !nome || !cpf || !rg || !profissao || !ccr || !assinatura) {
      res
        ...
        .json({
          error:
            "ID, Nome, cpf, rg, profissão, ccr e assinatura são obrigatórios!!!",
        });
      return;
    }
    const createdProfissional = await Profissional.create({
      id,
      nome,
      cpf,
      rg,
  ...
  static async updateProfissional(req, res) {
    ...
    if (!nome || !cpf || !rg || !profissao || !ccr || !assinatura) {
      res
        ...
        .json({
          error:
            "Nome, cpf, rg, profissão, ccr e assinatura são obrigatórios!!!",
        });
      return;
    }
    const updatedProfissional = await Profissional.update(
      { nome, cpf, rg, profissao, ccr, assinatura },
